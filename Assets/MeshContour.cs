...
public class MeshContour : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Vector4 temp = Camera.main.cameraToWorldMatrix * E;
        cameraFace = new Vector3(temp.x, temp.y, temp.z);
        ...
        for (i = 0; i < triangles.Length / 3; i++)
        {
            ...
            for (j = 0; j < i; j++)
            {
                ...
                if (doJudgeNorm)
                {
                    ...
                    //if (Vector3.Dot(normals[v1], cameraFace) * Vector3.Dot(normals[v2], cameraFace) < 0)
                    if (Vector3.Dot(normals[v1],normals[v2]) < 0)
                    {
                        //draw edge v1~v2
